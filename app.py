...
def roll_die(die, times, channel):
    ...
    response_text = "I rolled a {} sided 🎲 {} times for you . Your results are 🔮 {} !".format(
        str(die), str(times), ", ".join(results)
    )
    return post_message(response_text, channel)


def respond_to_help(channel):
    response_text = """Hello, I am a bot. I live to serve. 
    I will listen for messages tagging me as @bgg-bot followed by a command. 
...
if __name__ == "__main__":
    # To treat this as a listener for the Slack Events API, the URL must be verified, 
    # internet facing and respond to the challenge request made by slack. Deployed to the "internet" using ngrok.
    app.run("0.0.0.0")
